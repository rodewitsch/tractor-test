...
export default function (props: any) {
  ...
  return <TestScreen {...props} colors={colors}></TestScreen>;
}

class TestScreen extends React.Component {
  constructor(props: any) {
    super(props);
    this.currentCategory = props.route.params.category;
    this.selectedTicket = props.route.params.ticket;
    this.timer = null;
    this.state = {
      ...
      ticketNumber:
        this.selectedTicket != undefined
          ? this.selectedTicket
          : this.randomInteger(0, this.getCategoryTickets(this.currentCategory).default.length - 1),
      questionNumber: 0,
      timer: '10:00',
    };
    this.state.answers = new Array(
      this.getCategoryTickets(this.currentCategory).default[this.state.ticketNumber].length
    )
      .fill(null)
      .map((item, index) => ({
        rightAnswer: this.getQuestionItem(this.state.ticketNumber, index).rightAnswer - 1,
        userAnswer: null,
      }));
  ...
  componentDidUpdate() {
    if (this.props?.route?.params?.retry) {
      this.setState({
        examStatus: 'inProgress',
        ticketNumber: this.props.route.params.ticket,
        questionNumber: 0,
        timer: '10:00',
        answers: new Array(
          this.getCategoryTickets(this.props.route.params.category).default[
            this.props.route.params.ticket
          ].length
        )
          .fill(null)
          .map((item, index) => ({
            rightAnswer: this.getQuestionItem(this.props.route.params.ticket, index).rightAnswer - 1,
            userAnswer: null,
          })),
      });
      this.startTimer(600);
      this.backHandler = BackHandler.addEventListener('hardwareBackPress', this.handleBackPress);
      this.props.route.params.retry = false;
    }
  }
  ...
  setAnswer(number) {
    ...
    if (this.state.answers[this.state.questionNumber].userAnswer != null) {
      const EMPTY_ANSWER = this.state.answers.findIndex(
        (item, index) => item.userAnswer == null && index > this.state.questionNumber
      );
      if (this.state.questionNumber < 9 && EMPTY_ANSWER != -1) {
        this.setState({ questionNumber: EMPTY_ANSWER });
      } else {
        this.setState({
          questionNumber: this.state.answers.findIndex((item) => item.userAnswer == null),
        });
      }
    } else {
  ...
  getExamStatus() {
    if (this.state.answers.every((item) => item.userAnswer != null)) {
      if (this.state.answers.reduce((acc, item) => (item.userAnswer != item.rightAnswer ? (acc += 1) : acc), 0) > 1)
        return 'failed';
      else return 'passed';
    }
    return 'inProgress';
  ...
  getQuestionStatusColor(number) {
    ...
    if (this.state.answers[number].rightAnswer == this.state.answers[number].userAnswer)
  ...
  render() {
    return (
      <GestureRecognizer onSwipe={(direction, state) => this.onSwipe(direction, state)} style={this.styles.container}>
        <Modal
        ...
        <View style={this.styles.header}>
          <View style={this.styles.headerIcon}>
            ...
            <View style={this.styles.headerTitle}>
              <Text style={this.styles.headerTicketNumber}>Билет {this.state.ticketNumber + 1}</Text>
              <Text style={this.styles.headerCategory}>Категория {this.currentCategory.substr(0, 1)}</Text>
            </View>
        ...
        <View style={this.styles.questionArea}>
          <Text style={this.styles.questionText}>{this.getQuestionItem().questionText}</Text>
        </View>
